...
from .forms_basic import CommandForm, SSHInfoForm
...
def cmd_select(request):
    ...
    return render(
        request,
        "asct/command/select.html",
        {"commands": commands, "sshinfos": sshinfos},
    )
...
def serverinfo_export(request):
    ...
    return common_export(
        "server_info_list.xlsx", "Server Info", headers, ServerInfo, mapper
    )
